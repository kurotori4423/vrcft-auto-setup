...
- `Hybrid`: 通常パラメーターの左右・上下などの担当範囲が揃っている場合は通常系を使い、足りない場合はSimplified系で補います。既定値です。
- `Simplified`: Simplified Tracking系を優先します。左右別シェイプキーだけがある場合でも、対応するSimplifiedパラメーター1本のアニメーションとしてまとめて駆動します。
- `Detailed`: 通常パラメーターを優先します。細かなトラッキング入力を明示的に使いたい場合に向きます。
