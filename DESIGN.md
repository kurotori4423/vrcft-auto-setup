...
2. 検知結果から最適な AnimatorController (FX / Additiveレイヤー) を自動構築
3. Binary Parameter による同期ビット数削減オプション
4. Humanoid eye muscle を使った Additive 視線アニメーションを自動生成
...
└── Editor/
    ...
    ├── VrcftAnimatorGenerator.cs   … FX/Additiveコントローラー構築 (Direct BT / Binaryデコード / スムージング)
    ├── VrcftEyeLookGenerator.cs    … Humanoid eye muscle 用 EyeLookモーション生成
...
FX 用に `Animations/`, `Animations/Binary/`, `Animations/Smooth/`, `FX_FaceTracking.controller`, `Menu/`, `<アバター名>_VRCFT.prefab` を生成する。
EyeLook 有効時は Additive 用に `Additive/Additive_EyeTracking.controller`, `Additive/Animations/*`, `Additive/Masks/VRCFT_HeadOnly.mask` も生成する。
...
VRCFT OSC入力 (同期パラメーター: Float直値 or Binary Bool群)
  ...
  ↓ [FX制御レイヤー]
      ...
      Eye/Lip 駆動レイヤーの weight を切替。Lip は TrackingControl で Mouth=Animation⇔Tracking を切替

[Additive controller: EyeLook 有効時のみ]
VRCFT OSC入力 (EyeLeftX / EyeRightX / EyeY)
  ↓ Binaryデコードレイヤー (Binary有効時のみ。Eye系3パラメーターだけ複製)
  ↓ スムージングレイヤー (有効時のみ。Eye系3パラメーターだけ複製)
  ↓ VRCFT_EyeLook_Additive
      Head-only AvatarMask 付き。EyeTrackingActive で Native/VRCFT state を切替
      Native: VRCAnimatorTrackingControl Eyes=Tracking
      VRCFT: VRCAnimatorTrackingControl Eyes=Animation + Humanoid eye muscle クリップ
```

- WriteDefaults: 全生成ステート ON (Direct BT 構成の標準)。
- FX controller は MA MergeAnimator `layerType=FX` でマージする。
- EyeLook 用 Additive controller が生成された場合は、別の MA MergeAnimator `layerType=Additive` でマージする。
...
Body メッシュに ARKit 48種 (eyeLook系除く) が camelCase で存在。EyeLook は Humanoid eye muscle を Additive レイヤーで駆動する。
...
- Descriptor `customEyeLookSettings` / RotationStates / Eye_L/Eye_R ボーン回転には依存しない。
- `FT/v2/EyeLeftX` / `FT/v2/EyeRightX` / `FT/v2/EyeY` (-1..1) を Additive controller 内でも Binary デコード/スムージングし、Humanoid eye muscle へ出力する。
- 生成クリップは Transform ではなく Animator の Humanoid muscle カーブを持つ。
  - `Left Eye Down-Up`, `Right Eye Down-Up`: Up=+1, Down=-1, Straight=0
  - `Left Eye In-Out`: X=-1 で +1、X=+1 で -1 (参考テンプレートの Eye Left Rotation に合わせる)
  - `Right Eye In-Out`: X=-1 で -1、X=+1 で +1 (参考テンプレートの Eye Right Rotation に合わせる)
- Additive controller の EyeLook レイヤーには Head-only AvatarMask を設定する。
- EyeTrackingActive 時は Additive 側 TrackingControl で Eyes=Animation に切替、OFF時は Tracking に戻す。FX 側は Eye ブレンドシェイプレイヤーの weight 切替のみ行い、Eyes の TrackingControl は Additive 側に任せる。
...
- **Phase B**: クリップ/FX・Additive生成 + Binary + スムージング + EyeLook (タスク#8,9,10,11)
